/*
 * Portions copyright (C) 2020 Alexander Sychev
 */
...
screeninit(void)
{
	uchar *fb = fbinit(&xgscreen.r.max.x, &xgscreen.r.max.y, &xgscreen.depth, &xgscreen.chan);
	if(fb == nil)
		panic("can't initialise %dx%dx%d framebuffer\n",
			xgscreen.r.max.x, xgscreen.r.max.y, xgscreen.depth);
	xgscreen.clipr = xgscreen.r;
	memsetchan(&xgscreen, xgscreen.chan);
	conf.monitor = 1;
	xgdata.bdata = fb;
	xgdata.ref = 1;
	gscreen = &xgscreen;
...
screenwin(void)
{
	...
	orange = allocmemimage(Rect(0, 0, 1, 1), xgscreen.chan);
	orange->flags |= Frepl;
